...
def train_model(
    ...
    try:
        ...
        with mlflow.start_run(run_name=run_name) as run:
            ...
            # Alias @champion для загрузки models:/fraud_detection_rf@champion
            try:
                ...
                if versions:
                    latest = sorted(
                        versions, key=lambda v: int(v.version), reverse=True)[0]
                    client.set_registered_model_alias(
                        'fraud_detection_rf', 'champion', latest.version)
                    ...
                    log_message(
                        f'INFO: Alias @champion -> version {latest.version}')
            except Exception as alias_err:
                log_message(
                    f'WARN: Не удалось выставить alias champion: {alias_err}')
