...
class TransactionInput(BaseModel):
    ...
    tx_datetime: Annotated[
        str, Field(description='Дата и время транзакции, формат yyyy-MM-dd HH:mm:ss')
    ]
...
class PredictionOutput(BaseModel):
    ...
    prediction: Annotated[
        int, Field(description='Предсказанный класс (0 — легитимная, 1 — мошенническая)')
    ]
...
class PredictionRequest(BaseModel):
    transactions: Annotated[
        list[TransactionInput], Field(description='Список транзакций для предсказания')
    ]
